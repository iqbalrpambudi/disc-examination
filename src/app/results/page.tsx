...
import { downloadDISCReportPDF, formatListItems } from "@/utils/discReportGenerator";
import { DISCReportData } from "@/templates/discReportTemplate";
...
export default function ResultsPage() {
  ...
  useEffect(() => {
    if (isTestCompleted && userEmail) {
      setEmailStatus({
        success: true,
        message:
          "Your results have been recorded. You can also download them as PDF or send them again if needed.",
      });
    }
  ...
  // Handle PDF download using the new template-based approach
  const handleDownloadPDF = async () => {
    if (!dominantType || !profile) return;

    try {
      setIsDownloading(true);

      // Prepare data for the DISC report template
      const reportData: DISCReportData = {
        dominantType: `${profile.name} (${dominantType})`,
        dominantTypeDescription: profile.title,
        dominantTypeLetter: dominantType,
        profileDescription: profile.description,
        workStyleDescription: profile.workStyle,
        strengthsList: formatListItems(profile.strengths),
        challengesList: formatListItems(profile.challenges),
        dPercentage: percentages.D,
        iPercentage: percentages.I,
        sPercentage: percentages.S,
        cPercentage: percentages.C,
        email: userEmail || "N/A",
        testDate: formatDate(testStartTime),
        completionTime: formatDate(testEndTime),
        duration: formatDuration(testDuration),
      };

      // Generate and download PDF using the template
      await downloadDISCReportPDF(
        reportData,
        `DISC_Assessment_${dominantType}_${new Date().toISOString().split("T")[0]}.pdf`
      );
    } catch (error) {
  ...
  const handleSendEmail = async () => {
    if (!dominantType || !profile || !userEmail) return;
    ...
    try {
      ...
      // Generate PDF as base64 using the template
      const { generateDISCReportPDF } = await import("@/utils/discReportGenerator");
      const pdfBase64 = await generateDISCReportPDF(reportData);
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-4">
                    <div className="bg-red-500 h-4 rounded-full" style={{ width: `${percentages.D}%` }}></div>
                  </div>
                </div>
